...
### 1. 함수들이 공유하는 공통 데이터 레코드를 캡슐화
    ...
    - 공통 데이터가 레코드 구조로 묶여 있지 않다면 먼저 데이터를 하나로 묶기
...
### - 변환 함수로 묶으면 검색과 갱신을 일관된 장소에서 처리할 수 있고 로직 중복도 막을 수 있다.
...
const productPrice = priceList[orderData[0].split("_")[1]];
